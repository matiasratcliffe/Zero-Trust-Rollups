...
class TS3000Miner:
    ...
    def mineFragment(self, requestID, difficulty):
        ...
        (fragmentIndex, globalHash, localHash, timestampRestriction) = decode(["("+",".join(re.findall(memberRegex, TS3000Contract.getInputDataStructure()))+")"], request["input"])[0]
        for passcode in range(10**difficulty):
            if keccak(encode(["uint", "bytes32"], [passcode, localHash])) == globalHash:
                break
        return encode(["("+",".join(re.findall(memberRegex, TS3000Contract.getResultDataStructure()))+")"], [(fragmentIndex, timestampRestriction, passcode)])
